...
public class MainActivity extends Activity {
    ...
    private void setupAdapters() {
        listAdapter = new ArrayAdapter<>(this, android.R.layout.simple_list_item_1, productLines());
    ...
    private void addProduct() {
        ...
        if (Double.isNaN(defaultUsage) || defaultUsage <= 0) {
            showToast("مصرف پیش‌فرض باید عددی بزرگ‌تر از صفر باشد.");
            return;
        }
    ...
    private void calculateDifference() {
        if (products.isEmpty()) {
            resultTextView.setText("اول حداقل یک محصول اضافه کنید.");
            return;
        }
        ...
        if (product == null) {
            resultTextView.setText("محصول معتبر را از نتایج جستجو انتخاب کنید.");
            return;
        }
        ...
        String result = "ماه: " + month + "\n"
                + "محصول: " + product.name + "\n"
                + "مصرف پیش‌فرض هر عدد: " + formatNumber(product.defaultUsage) + " پا\n"
                + "تعداد: " + formatNumber(quantity) + "\n"
                + "مصرف استاندارد کل: " + formatNumber(expectedLeather) + " پا\n"
                + "چرم مصرفی واقعی: " + formatNumber(actualLeather) + " پا\n"
                + "مابه‌التفاوت: " + formatSignedNumber(difference) + " پا\n"
                + "وضعیت: " + status;

        resultTextView.setText(result);
        saveCalculation(month, product, quantity, actualLeather, expectedLeather, difference, status);
        showToast("محاسبه در سوابق ماه «" + month + "» ذخیره شد.");
    }
    ...
    private void refreshAdapters() {
        listAdapter.clear();
        listAdapter.addAll(productLines());
    ...
    private List<String> productLines() {
        ...
        for (Product product : products) {
            lines.add("نام محصول: " + product.name + "\nمصرف پیش‌فرض: " + formatNumber(product.defaultUsage) + " پا");
        }
        return lines;
    }
